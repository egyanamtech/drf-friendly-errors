...
class ExceptionHandlerTestCase(BaseTestCase):
    ...
    def test_server_error(self):
        ...
        self.assertEqual(
            response.data["code"], settings.FRIENDLY_EXCEPTION_DICT.get("APIException")
        )

    def test_handler_do_not_touch_pretty_errors(self):
        self.data_set["language"] = "node.js"
        request = self.factory.post(reverse("api:snippet-list"), data=self.data_set)
    ...
    def test_not_found(self):
        ...
        self.assertEqual(
            response.data["code"], settings.FRIENDLY_EXCEPTION_DICT.get("NotFound")
        )
